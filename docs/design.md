...
- SBOM (CycloneDX) and SLSA provenance attestations are generated by `docker/build-push-action` (`sbom: true`, `provenance: true`) and stored as OCI referrers alongside the image manifest. They do not change image digests or tag behaviour.
